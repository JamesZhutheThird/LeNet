...
class avg_pooling():
    ...
    def forward(feature, size=2, stride=2):
        ...
        pool_out = np.zeros((B, C, out_h, out_w), dtype=np.float32)

        for b in np.arange(B):
            for c in np.arange(C):
                for i in np.arange(out_h):
                    for j in np.arange(out_w):
                        pool_out[b, c, i, j] = np.mean(feature[b, c,
...
class LeNet(object):
    ...
    def forward(self, x):
        ...
        for i in range(B):
            #第一次卷积
            conv1 = relu.forward(add_bias(conv.forward(x[i,:,:] , self.kernels[0]), self.kernels_biases[0]))
            pool1 = avg_pooling.forward(conv1)
            
            #第二次卷积
            conv2 = relu.forward(add_bias(conv.forward(pool1[i,:,:] , self.kernels[0]), self.kernels_biases[1]))
            pool2 = avg_pooling.forward(conv1)
